...
static const char *TAG = "myflash";
static char spiffs_stat = false;
...
int storage_flash_init(void)
{
    ...
    if (spiffs_stat) {
        ESP_LOGW(TAG, "SPIFFS is already mounted!");
        return 0;
    }
    ...
    if (ret != ESP_OK) {
        ...
        ESP_LOGI(TAG, "Partition for user size: total: %d, used: %d", total, used);
    }
    spiffs_stat = true;
...
int storage_flash_deinit(void)
{
    ...
    if (!spiffs_stat) {
        ESP_LOGW(TAG, "SPIFFS is already unmounted!");
        return 0;
    }
